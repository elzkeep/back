...
func (p *Omar) Init(tile resources.TileItem) {
	p.InitFaction("Omar", "Omar", resources.GetFactionTile(resources.TileFactionOmar), tile)
}
...
func (p *Omar) Build(x int, y int, needSpade int, building resources.Building) error {
...
func (p *Omar) Book(item action.BookActionItem, book resources.Book) error {
