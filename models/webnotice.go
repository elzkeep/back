...
func (p *WebnoticeManager) GetQuery() string {
    ...
    str := "select wn_id, wn_title, wn_content, wn_image, wm_category, wm_date from webnotice_tb "
...
func (p *WebnoticeManager) Insert(item *Webnotice) error {
    ...
    var err error
    if item.Id > 0 {
        query = "insert into webnotice_tb (wn_id, wn_title, wn_content, wn_image, wm_category, wm_date) values (?, ?, ?, ?, ?, ?)"
        res, err = p.Exec(query , item.Id, item.Title, item.Content, item.Image, item.Category, item.Date)
    } else {
        query = "insert into webnotice_tb (wn_title, wn_content, wn_image, wm_category, wm_date) values (?, ?, ?, ?, ?)"
...
func (p *WebnoticeManager) Delete(id int64) error {
    ...
    query := "delete from webnotice_tb where wm_id = ?"
...
func (p *WebnoticeManager) DeleteWhere(args []interface{}) error {
    ...
    for _, arg := range args {
        ...
        case Where:
            ...
            if item.Compare == "in" {
                query += " and wm_" + item.Column + " in (" + strings.Trim(strings.Replace(fmt.Sprint(item.Value), " ", ", ", -1), "[]") + ")"
            } else if item.Compare == "between" {
                query += " and wm_" + item.Column + " between ? and ?"

                s := item.Value.([2]string)
                params = append(params, s[0])
                params = append(params, s[1])
            } else {
                query += " and wm_" + item.Column + " " + item.Compare + " ?"
...
func (p *WebnoticeManager) Update(item *Webnotice) error {
	...
	query := "update webnotice_tb set wn_title = ?, wn_content = ?, wn_image = ?, wm_category = ?, wm_date = ? where wm_id = ?"
...
func (p *WebnoticeManager) UpdateTitle(value string, id int64) error {
	...
	query := "update webnotice_tb set wm_title = ? where wm_id = ?"
...
func (p *WebnoticeManager) UpdateContent(value string, id int64) error {
	...
	query := "update webnotice_tb set wm_content = ? where wm_id = ?"
	_, err := p.Exec(query, value, id)
...
func (p *WebnoticeManager) UpdateImage(value string, id int64) error {
	...
	query := "update webnotice_tb set wm_image = ? where wm_id = ?"
	_, err := p.Exec(query, value, id)
...
func (p *WebnoticeManager) UpdateCategory(value int, id int64) error {
	...
	query := "update webnotice_tb set wm_category = ? where wm_id = ?"
	_, err := p.Exec(query, value, id)
...
func (p *WebnoticeManager) IncreaseCategory(value int, id int64) error {
	...
	query := "update webnotice_tb set wm_category = wm_category + ? where wm_id = ?"
	_, err := p.Exec(query, value, id)
