...
func (p *BillingManager) Insert(item *Billing) error {
    ...
    var err error
    if item.Id > 0 {
        query = "insert into billing_tb (bi_id, bi_price, bi_status, bi_giro, bi_billdate, bi_month, bi_period, bi_company, bi_building, bi_date) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
        res, err = p.Exec(query , item.Id, item.Price, item.Status, item.Giro, item.Billdate, item.Month, item.Period, item.Company, item.Building, item.Date)
    } else {
        query = "insert into billing_tb (bi_price, bi_status, bi_giro, bi_billdate, bi_month, bi_period, bi_company, bi_building, bi_date) values (?, ?, ?, ?, ?, ?, ?, ?, ?)"
        res, err = p.Exec(query , item.Price, item.Status, item.Giro, item.Billdate, item.Month, item.Period, item.Company, item.Building, item.Date)
    }
...
func (p *BillingManager) Update(item *Billing) error {
	...
	query := "update billing_tb set bi_price = ?, bi_status = ?, bi_giro = ?, bi_billdate = ?, bi_month = ?, bi_period = ?, bi_company = ?, bi_building = ?, bi_date = ? where bi_id = ?"
	_, err := p.Exec(query , item.Price, item.Status, item.Giro, item.Billdate, item.Month, item.Period, item.Company, item.Building, item.Date, item.Id)
