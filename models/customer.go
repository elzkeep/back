...
type Customer struct {
    ...
    Usevat                int `json:"usevat"`         
    Contracttotalprice                int `json:"contracttotalprice"`         
...
func (p *CustomerManager) Update(item *Customer) error {
	...
	query := "update customer_tb set cu_number = ?, cu_kepconumber = ?, cu_kesconumber = ?, cu_type = ?, cu_checkdate = ?, cu_managername = ?, cu_managertel = ?, cu_manageremail = ?, cu_contractstartdate = ?, cu_contractenddate = ?, cu_usevat = ?, cu_contracttotalprice = ?, cu_contractprice = ?, cu_contractvat = ?, cu_contractday = ?, cu_contracttype = ?, cu_billingdate = ?, cu_billingtype = ?, cu_billingname = ?, cu_billingtel = ?, cu_billingemail = ?, cu_address = ?, cu_addressetc = ?, cu_collectmonth = ?, cu_collectday = ?, cu_manager = ?, cu_tel = ?, cu_fax = ?, cu_periodic = ?, cu_lastdate = ?, cu_remark = ?, cu_status = ?, cu_salesuser = ?, cu_user = ?, cu_company = ?, cu_building = ?, cu_date = ? where cu_id = ?"
...
func (p *CustomerManager) UpdateUsevat(value int, id int64) error {
    if p.Conn == nil && p.Tx == nil {
        return errors.New("Connection Error")
    }

	query := "update customer_tb set cu_usevat = ? where cu_id = ?"
	_, err := p.Exec(query, value, id)

    return err
}

func (p *CustomerManager) UpdateContracttotalprice(value int, id int64) error {
    if p.Conn == nil && p.Tx == nil {
        return errors.New("Connection Error")
    }

	query := "update customer_tb set cu_contracttotalprice = ? where cu_id = ?"
	_, err := p.Exec(query, value, id)
...
func (p *CustomerManager) IncreaseUsevat(value int, id int64) error {
    if p.Conn == nil && p.Tx == nil {
        return errors.New("Connection Error")
    }

	query := "update customer_tb set cu_usevat = cu_usevat + ? where cu_id = ?"
	_, err := p.Exec(query, value, id)

    return err
}

func (p *CustomerManager) IncreaseContracttotalprice(value int, id int64) error {
    if p.Conn == nil && p.Tx == nil {
        return errors.New("Connection Error")
    }

	query := "update customer_tb set cu_contracttotalprice = cu_contracttotalprice + ? where cu_id = ?"
	_, err := p.Exec(query, value, id)
