...
create view statistics_vw as
select gu_faction, gu_color, sum(if(gu_rank = 1, 1, 0))as gu_rank1, sum(if(gu_rank = 2, 1, 0)) as gu_rank2, sum(if(gu_rank = 3, 1, 0)) as gu_rank3, sum(if(gu_rank = 4, 1, 0)) as gu_rank4, sum(if(gu_rank = 5, 1, 0)) as gu_rank5, count(*) as gu_count, convert(avg(gu_score), int) as gu_avg from game_tb, gameuser_tb where g_status = 4 and g_id = gu_game group by gu_faction, gu_color;
